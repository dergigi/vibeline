...
Below are individual summaries from voice memos recorded throughout the month, listed chronologically with their timestamps:
...
def main() -> None:
    ...
    parser.add_argument(
        "-m", "--month",
        help="Specific month to process (YYYY-MM format). Default: all months."
    )
    parser.add_argument(
        "-f", "--force",
        action="store_true",
        help="Force regenerate existing summaries"
    )
    parser.add_argument(
        "-n", "--dry-run",
        action="store_true",
